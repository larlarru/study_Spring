...
<!-- 							<button type="button" id="memberRegistBtn" class="btn btn-primary" onclick="OpenWindow(&#39;registForm.do&#39;,&#39;회원등록&#39;,800,700);">회원등록</button> -->
							<a href="${cp }/member/registView">
								<button type="button" id="memberRegistBtn" 
								class="btn btn-primary" >회원등록</button>
							</a>
							<div id="keyword" class="card-tools" style="width: 550px;">
							
							<form id="#frm2" action="${cp }/member/pagingSearchMember" method="get">
...
<!-- 										<button id="selectTextBtn" class="btn btn-primary" type="button" id="searchBtn" data-card-widget="search" onclick="searchList_go(1);"> -->
									...
									<table class="table table-bordered">
										<tbody>
											<c:forEach items="${memberList }" var="member">
												<tr>
													<th><a href="${cp }/member/memberInfo?userid=${member.userid }">${member.userid }</a></th>
													<th>${member.pass }</th>
													<th>${member.alias }</th>
													<th>${member.addr1 }</th>
						...
						<div class="card-footer">
							<nav aria-label="member list Navigation">
								<ul class="pagination justify-content-center m-0">
									<li class="page-item"><a class="page-link" href="${cp }/member/pagingSearchMember?page=1&pageSize=${pageVo.pageSize}"><i class="fas fa-angle-double-left"></i></a></li>
									
									<c:choose>
										<c:when test="${pageVo.page > 1 }">
											<li class="page-item"><a class="page-link" href="${cp }/member/pagingSearchMember?page=${pageVo.page-1 }&pageSize=${pageVo.pageSize}"><i class="fas fa-angle-left"></i></a></li>
									</c:when>
										<c:otherwise>
	...
	<%-- 														<li class="page-item active"><a class="page-link" href="#">${i }</a></li> --%>
														...
														<c:otherwise>
															<li><a class="page-link" href="${cp }/member/pagingSearchMember?page=${i }&pageSize=${pageVo.pageSize}">${i }</a></li>
<%-- 															<li><a class="page-link" href="#">${i }</a></li> --%>
									...
									<c:choose>
										<c:when test="${pageVo.page == pagination}">
											<li class="page-item"><a class="page-link" href="${cp }/member/pagingSearchMember?page=${pagination }&pageSize=${pageVo.pageSize}"><i class="fas fa-angle-right"></i></a></li>
									</c:when>
										<c:otherwise>
											<li class="page-item"><a class="page-link" href="${cp }/member/pagingSearchMember?page=${pageVo.page+1 }&pageSize=${pageVo.pageSize}"><i class="fas fa-angle-right"></i></a></li>
										</c:otherwise>
									</c:choose>
									<li class="page-item"><a class="page-link" href="${cp }/member/pagingSearchMember?page=${pagination }&pageSize=${pageVo.pageSize}"><i class="fas fa-angle-double-right"></i></a></li>
								</ul>
							</nav>
